...
class InputBar extends Component {
    ...
    toggleTaskDone(id){
        ...
        this.setState({taskDone: newTaskDone }, console.log(this.state.taskDone,  newTaskDone))
      ...
      <div>
        <div className='field'>
          <div className="input">
            ...
            <button className="clearButton" onClick={this.clearTask}>Clear task</button>
          </div>
          <div className='fullList'>
            <div>
                <p className='listTitle'>TO-DO LIST</p>
                    <div className='todolist'>
                        <p className='tasks'>{ this.state.TodoList.length >= 1 ? this.state.TodoList.map(el => {
                            return <List  task={el.task}
                                            key={el.id}
                                            id={el.id}
            ...
            <div>
                <p className='listTitle'>{this.state.taskDone > 1 ? 'TASKS COMPLETED' : 'TASK COMPLETED'}</p>
                <div className='todolistDone'>
                        <p className='tasks'>{ this.state.taskDone.length >= 1 ? this.state.taskDone.map(el => { 
                            return <ListTaskDone  task={el.task}
                                                    key={el.id}
                                                    id={el.id}
                                                    complete={el.complete} 
